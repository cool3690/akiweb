	...
	<div class="container" style="padding-bottom:8%;">
		<?php 
			...
			while ($cart_row = mysqli_fetch_array($cart_sql)) {
				...
				echo '<form method="post" name="form1" id="form1" action="course_cart.php">';	
				$course_num = $cart_row['course_num'];
				$course_sql = mysqli_query($db, "SELECT * FROM course where course_num = '$course_num' and exist='0'");	
				...
				//刪除課程
					...
					foreach($check as $value){	
						...
						if (mysqli_query($db, $cart_delete_sql)) {
							...
							echo "window.location.href='course_cart.php'";
							echo "</script>"; 
						} else {
							echo "更新失敗";
						}	
