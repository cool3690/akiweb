	...
	if (isset($_POST['submit'])) {
		...
		if($submit==2){			
			if($passwdo1==''){
				...
				if(@$_SESSION['admin']=="Y"){
					$sql = mysqli_query($db, "SELECT * FROM administrator where account = '$account' and passwd = '$passwdo1'");
					$sql2 = "UPDATE administrator SET passwd='$passwdn1',name='$name', email='$email' WHERE account='$account'"; 
				}else{
					$sql = mysqli_query($db, "SELECT * FROM user where account = '$account' and passwd = '$passwdo1'");
					$sql2 = "UPDATE user SET passwd='$passwdn1',name='$name', email='$email' WHERE account='$account'"; 
				}
