			...
			<div class="mt-5">
				<?php
					...
					if(@$_SESSION['name']==null){						
						...
						$name = $_SESSION['name'];
						$name = $_SESSION['name'];
						$select_order = mysqli_query($db, "SELECT * FROM clsm where business_name = '$name'");
						...
						while ($row = mysqli_fetch_array($select_order)) {
							echo '<tbody><tr>				
							  ...
							  <td class="align-middle">
								<form method="post" name="form1" id="form1" action="clsm_update.php" target="_blank">
									<input type="hidden" name="num" value="'.$row['num'].'">
									<button type="submit" class="btn btn-info" name="submit">PDF</button></div>
								</form>
							  </td>		
							  <td class="align-middle">
								<form method="post" name="form1" id="form1" action="report_clsm.php" target="_blank">
									<input type="hidden" name="num" value="'.$row['num'].'">
