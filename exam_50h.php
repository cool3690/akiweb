	...
	<div class="container mt-5">
		...
		<?php
				...
				if (isset($_POST['submit'])) {
					$id=$_POST['id'];
					$submit=$_POST['submit'];
					$quiz_sql = mysqli_query($db, "SELECT * FROM basic where id='$id'");
					if($submit==0){
				...
				$result = mysqli_query( $db,"SELECT * FROM basic");
...
';
							$result = mysqli_query($db, "SELECT * FROM basic50 a left join 50h b on a.id=b.id where a.id='$amount'");
							$h = mysqli_query($db, "SELECT * FROM 50h");
...
echo '
									<div class="row text-center">
										<div class="col-12 col-xs-12 col-sm-12 col-md-4 col-lg-4">
											<img src="images/drawable/50h/'.$pronunciation.'.gif" height="200" id="gif" style="display:none;">
											<img src="images/drawable/50h/'.$pronunciation.'.png" height="180" width="180" id="img" style="display:none;"><br>
											<i class="far fa-play-circle mt-5" id="replaygif" style="color:#0072E3;display:none;"></i>
										</div>
										<div class="col-12 col-xs-12 col-sm-12 col-md-8 col-lg-5">
											<img src="images/drawable/50h/'.$pronunciation.'_img.png" height="200">
...
';
													while ($hrow = mysqli_fetch_array($h)) {
														echo '<option value="'.$hrow['id'].'"'; echo $amount==$hrow['id']?'selected':''; echo'>'.$hrow['word'].'</option>';
													}
echo '
												</select>
...
';					
								$gifFilePath = 'images/drawable/50h/'.$pronunciation.'.gif';
				...
				$min=$amount-1;
				$max=$amount+1;
				$word_min_sql = mysqli_query($db, "SELECT * FROM 50h where id='$min'");
				while ($word_min_row = mysqli_fetch_array($word_min_sql)) {
					$word_min=$word_min_row['word'];
				}
				$word_max_sql = mysqli_query($db, "SELECT * FROM 50h where id='$max'");
